...
Scans the Windows registry to detect installed Autodesk AEC Collection products (2019–2027), then — after showing a full pre-flight summary and requiring explicit confirmation — uninstalls them silently using ODIS (2022+) or MSI (legacy), removes the Autodesk Desktop Licensing Service, deletes all Autodesk registry keys, and cleans up remaining directories across Program Files, ProgramData, and all user profiles. A timestamped log is always written automatically.
